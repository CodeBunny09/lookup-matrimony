...
if($count==0){
	...
	$gender=$_POST['gender'];
	$mid = $matri_id;
	$sel_own_data=$DatabaseCo->dbLink->query("select birthdate,ocp_name,height,city_name,country_name,state_name,photo1_approve,photo1,photo_view_status,photo_protect,photo_pswd,gender,username,matri_id,religion_name,caste_name,edu_detail,fstatus,index_id,status,tokan,firstname,lastname,caste,part_caste from register_view where matri_id!='$matri_id' and gender!='$gender' and status!='Inactive' and status!='Suspendade' and fstatus='Featured' order by reg_date desc");
