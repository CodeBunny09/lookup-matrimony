...
if (mysqli_num_rows($DatabaseCo->dbResult) > 0) {
    ...
    $rows = mysqli_num_rows($DatabaseCo->dbLink->query("SELECT matri_id FROM register_view where (find_in_set($country, part_country_living) > 0) AND (find_in_set($rel, part_religion) > 0) AND gender!='$gen'"));
	
    $sql = $DatabaseCo->dbLink->query("select * from register_view where (find_in_set($country, part_country_living) > 0) AND (find_in_set($rel, part_religion) > 0) AND gender!='$gen' order by fstatus desc");
    $count = 0;
    if ($rows > 0) {
