   ...
   // $subcaste = $DatabaseCo->dbRow->subcaste;
    ...
    $part_mtongue = $DatabaseCo->dbRow->moonsign;
        ...
        // Photo 1 as 5% value
         ...
         if($photo2 != '' && $photo3 != '' && $photo4 != '' && $photo5 != '' && $photo6 != '')   {    $photo2 = '10';   }else{    $photo2 = '0';       }
        ...
        $hasCompletedVideo = ($cpass_status);
        $hasCompletedBasic = ($fname+$lname+$mstatus+$mtongue+$bdate+$btime+$bplace+$hobby+$profile_text);
        $hasCompletedEdu = ($emp+$occ+$income+$edu);
        $hasCompletedCont = ($state+$city+$country+$mobile);
        $hasCompletedSocial = ($religion+$caste+$horoscope+$manglik+$star+$moonsing+$part_edu);
        ...
        $hasCompletedFamily = ($father_name+$mother_name+$father_occupation+$mother_occupation+$no_of_sisters+$family_type+$family_status+$no_of_brothers+$no_marri_brothers+$no_marri_sisters);
