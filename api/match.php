...
if (mysqli_num_rows($DatabaseCo->dbResult) > 0) {
    ...
    $my_caste=$DatabaseCo->dbRow->caste;
	$part_caste=$DatabaseCo->dbRow->part_caste;
	if($DatabaseCo->dbRow->will_to_mary_caste == '0'){
    ...
    $rows = mysqli_num_rows($DatabaseCo->dbLink->query("select matri_id from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $a $i"));
    $sql = $DatabaseCo->dbLink->query("select * from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $a  $i order by fstatus desc");
    $count = 0;
    if ($rows > 0) {
