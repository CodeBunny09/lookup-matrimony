...
if (mysqli_num_rows($DatabaseCo->dbResult) > 0) {
    ...
    //$rows = mysqli_num_rows($DatabaseCo->dbLink->query("select matri_id from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $a "));
	$rows = mysqli_num_rows($DatabaseCo->dbLink->query("select matri_id from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $d $a "));
   // $sql = $DatabaseCo->dbLink->query("select * from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $a order by fstatus desc");
	$sql = $DatabaseCo->dbLink->query("select * from register_view where  status!='Inactive' and status!='Suspended' and gender!='" . $gender . "' $g $h $c $d $a order by fstatus desc");
    $count = 0;
    if ($rows > 0) {
