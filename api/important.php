...
if($count==0){
	$matri_id=$_POST['matri_id'];
	$get_msg=$DatabaseCo->dbLink->query("select * from messages where msg_important_status='Yes' and trash_sender='No' and (from_id='$matri_id' OR to_id='$matri_id') order by mes_id desc");
	
	if (mysqli_num_rows($get_msg) > 0) {
		$count=0;
		while ($contact_res = mysqli_fetch_object($get_msg)) {
			
			$get_det=$DatabaseCo->dbLink->query("SELECT * from register WHERE matri_id='$matri_id'");
