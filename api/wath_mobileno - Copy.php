...
if($count==0){
	...
	$rows=mysqli_num_rows($DatabaseCo->dbLink->query("SELECT * FROM contact_view JOIN register_view ON contact_view.my_id=register_view.matri_id WHERE contact_view.viewed_mem_id='$matri_id' AND gender!='$gender' ORDER BY con_id DESC ")); 
	$sql = "SELECT * FROM contact_view JOIN register_view ON contact_view.my_id=register_view.matri_id WHERE contact_view.viewed_mem_id='$matri_id' AND gender!='$gender' ORDER BY con_id DESC ";
	...
	if ($rows > 0) {
		...
		while ($contact_res =  mysqli_fetch_object($data)) {
			...
			$count++;
			$response['responseData'][$count] = array('matri_id' => $contact_res->matri_id,'birthdate' => $contact_res->birthdate,'ocp_name' => "$occu_name",'height' => $contact_res->height,'city_name' => "$city8",'country_name' => "$country_name8",'photo1_approve' => $contact_res->photo1_approve,'photo_view_status' => $contact_res->photo_view_status,'photo_protect' => $contact_res->photo_protect,'photo_pswd' => $contact_res->photo_pswd,'gender' => $contact_res->gender,'username' => "$name",'is_shortlisted' => "$sh",'is_blocked' => "$block",'is_favourite' =>"$mes",'user_profile_picture' => $photo,'profile_text'=>"$all",'status'=>"1",'tokan'=>"$tokan");
