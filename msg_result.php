...
{
$get_msg=$DatabaseCo->dbLink->query("select * from messages where msg_status='sent' and from_id='".$_SESSION['user_id']."' and msg_read_status='Yes' and trash_sender='No' order by mes_id desc");
}
else if(isset($_POST['msg_read_type']) && $_POST['msg_read_type']=='unread')
{
$get_msg=$DatabaseCo->dbLink->query("select * from messages where msg_status='sent' and from_id='".$_SESSION['user_id']."' and msg_read_status='No' and trash_sender='No' order by mes_id desc");
}
else if(isset($_POST['msg_read_type']) && $_POST['msg_read_type']=='read_all')
{
$get_msg=$DatabaseCo->dbLink->query("select * from messages where msg_status='sent' and from_id='".$_SESSION['user_id']."' and trash_sender='No' order by mes_id desc");	
}
else
{
$get_msg=$DatabaseCo->dbLink->query("select * from messages where msg_status='sent' and from_id='".$_SESSION['user_id']."' and trash_sender='No' order by mes_id desc");	
...
{
?>
    <?php include "parts/main_msg_sent.php" ;?>
    <?php
}
?>
  </ul>
