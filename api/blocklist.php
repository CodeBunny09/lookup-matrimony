...
if($count==0){
	...
	$rows = mysqli_num_rows($DatabaseCo->dbLink->query("SELECT * FROM register_view JOIN block_profile ON block_profile.block_to=register_view.matri_id WHERE block_by='$matri_id' AND gender!='$gender' "));
	
	$sql = "SELECT * FROM register_view JOIN block_profile ON block_profile.block_to=register_view.matri_id WHERE block_by='$matri_id' AND gender!='$gender' ORDER BY reg_date desc"; 
