...
if (mysqli_num_rows($DatabaseCo->dbResult) > 0) {
	...
	$hei=$DatabaseCo->dbRow->height;
	$con=$DatabaseCo->dbRow->country_id;
	$rel=$DatabaseCo->dbRow->religion;
	$e=$DatabaseCo->dbRow->birthdate;
	$gen=$DatabaseCo->dbRow->gender;
	$caste=$DatabaseCo->dbRow->caste;
	$mat=$DatabaseCo->dbRow->m_status;
    ...
    $sql = $DatabaseCo->dbLink->query("select * from register_view where (find_in_set($con, part_country_living) > 0) AND part_height<='$hei' AND (find_in_set($rel, part_religion) > 0) AND (find_in_set($edu_main, part_edu) > 0) AND gender!='$gen' AND (find_in_set($caste, part_caste) > 0) AND part_frm_age<=$age order by fstatus desc");
    $count = 0;
    if ($rows > 0) {
