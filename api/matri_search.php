...
if($count==0 ){
	...
	$login_id=$_POST['login_matri_id'];
	$gender=$_POST['gender'];
	if($login_id!=$matri_id){
	$GET_MEMBER_DATA = $DatabaseCo->dbLink->query("SELECT * FROM register_view WHERE matri_id='$matri_id' AND gender!='$gender'");
	if (mysqli_num_rows($GET_MEMBER_DATA) > 0) {
		$count=0;
		while ($contact_res = mysqli_fetch_object($GET_MEMBER_DATA )) {      
			$count++;
			...
			if($site_data->username_setting == 'full_username'){
      		...
      		$response['responseData'][$count] = array('user_id' => $contact_res->index_id,'matri_id' => $contact_res->matri_id,'birthdate' => $contact_res->birthdate,'photo1_approve' => $contact_res->photo1_approve,'photo_view_status' => $contact_res->photo_view_status,'photo_protect' => $contact_res->photo_protect,'photo_pswd' => $contact_res->photo_pswd,'gender' => $contact_res->gender,'username' => "$name",'is_shortlisted' => "$sh",'is_blocked' => "$block",'is_favourite' => "$mes",'user_profile_picture' =>"$photo",'profile_text'=>"$all",'caste' =>"$ca",'edu_detail' =>"$add_fname",'addition_detail' =>"$edu_fname", 'state_name' => "$state8",'country_name' =>"$country_name8",'religion_name' =>"$re",'city_name' => "$city8",'age'=>"$age",'ocp_name' =>"$ocp8",'height' =>"$height",'member_status'=>"$st",'status'=>"1",'tokan'=>"$tokan");
		}
		$response['status'] = "1";
	...
	$response['status'] = "0";
	$response['message'] = "Please Enter Valid Matri id";
	echo json_encode($response);
}
}else{
	$response['status'] = "0";
